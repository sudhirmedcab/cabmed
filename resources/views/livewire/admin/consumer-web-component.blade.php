        ...
        <div class="card text-center mt-2 mb-2">
          <div class="custom__driver__filter card-header">
            <ul class="nav nav-tabs custom__nav__tab  card-header-tabs d-flex justify-content-around overflow-auto flex-nowrap">
                ...
                <li href="/consumer-enquiry" class="nav-item {{$this->activeTab == 'app' ? 'active':''}}">
                    <a class="custom__nav__btn nav-link fs-1" wire:click="filterCondition('app')">
                    App
                    </a>
                </li>
                <li href="/consumer-web" class="nav-item {{$this->activeTab == 'web' ? 'active':''}}">
                    <a class="custom__nav__btn nav-link fs-1" wire:click="filterCondition('web')">
                    Web
                    </a>
                </li>
                <li  href="/consumer-dial" class="nav-item {{$this->activeTab == 'dial' ? 'active':''}}">
                    <a class="custom__nav__btn nav-link fs-1" wire:click="filterCondition('dial')">
                    Dial
                    </a>
                </li>
        ...
        <div class="card mt-2">
            <div class="card-header custom__filter__select">
                ...
                <div class="col __col-3">
                    ...
                    <div class="col __col-3">
                        <div class="form-group">
                            <label class="custom__label">Services By Status</label>
                            <select wire:model.live.debounce.150ms="ambulanceServiceFiter" wire:mode.live="ambulanceServiceFiter" class="custom__input__field custom-select rounded-0 form-control form-control-sm" id="SelectUnderVerification">
                                <option value="">Select name</option>

                                <option value="roadAmbulance">Road Ambulance</option>
                                <option value="deadbodyAmbulance">Dead Body Ambulance</option>
                                <option value="trainAmbulance">Train Ambulance</option>
            ...
            <div wire:loading.remove wire:target="filterCondition" class="card-body p-2 overflow-auto">
                <table class="table custom__table table-bordered table-sm">
                    ...
                    @if(!empty($medcabWebRecords))

                    @foreach ($medcabWebRecords as $consumer)

                    <tr>
                        <td>{{ $srno }}</td>
                        <td>{{$consumer->enquiry_id }}</td>
                        <td>{{date('j F, Y h:i A', strtotime($consumer->created_at))}}</td>
                        <td>{{$consumer->enquiry_name}}</td>
                        <td>{{$consumer->enquiry_mobile}}</td>
                        <td>{{$consumer->enquiry_ambulance}}</td>
                        <td>{{$consumer->city_name}}</td>
                        <td>{{$consumer->state_name}}</td>
                        <td>
                            @if(($consumer->remark_id))
                            <input type="text" value="{{$consumer->remark_text}}" class="text-center">
                            @else
                        ...
                        <td class="action__btn lbtn-group">
                            ...
                            <button wire:confirm="Are you sure you want to delete this Consumer Web Enquiry?" wire:click="deleteconsumerWeb({{ $consumer->enquiry_id }})" class="btn-danger"><i class="fa fa-trash fa-sm"></i></button>
                            <button wire:confirm="Are you sure you want to save this as a consumer data ?" wire:click="SaveconsumerWeb({{ $consumer->enquiry_id }})" class="btn-primary"><i class="fa fa-check fa-sm"></i></button>
                        </td>
                    </tr>
                    @php
                    $srno++
                ...
                <div class="custom__pagination mt-2 pt-1 card-footer__ clearfix">
                    {{ $medcabWebRecords->links() }}
                </div>
            </div>
        </div>

        <div class="container">
            <div class="row" wire:loading wire:target="selectedDate,ambulanceServiceFiter,filterCondition" wire:key="selectedDate,ambulanceServiceFiter,filterCondition">
                <div class="col">
                    <div class="loader">
