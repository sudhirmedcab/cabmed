        ...
        <div class="card mt-2">
                ...
                <div class="col __col-3">
                    ...
                    <div class="col __col-3">
                        <div class="form-group">
                            ...
                            <select wire:model.live.debounce.150ms="mainServiceFiter" wire:mode.live="mainServiceFiter" class="custom__input__field custom-select rounded-0 form-control form-control-sm" id="SelectUnderVerification">
                                <option value="">Select name</option>
                                @foreach ($mainServices as $list)
                                <option value="{{$list->ams_id}}">{{$list->ams_name}}</option>
                                @endforeach
                             </select>
            ...
            <div wire:loading.remove wire:target="filterCondition" class="card-body p-2 overflow-auto">
                <table class="table custom__table table-bordered table-sm">
                    ...
                    <tr>
                        <td>{{ $srno }}</td>
                        <td>{{$consumer->genral_enquary_id }}</td>
                        <td>{{date('j F, Y h:i A', $consumer->genral_enquary_time)}}</td>
                        <td>{{$consumer->genral_enquary_consumer_name}}</td>
                        <td>{{$consumer->genral_enquary_consumer_mobile}}</td>
                        <td>{{$consumer->genral_enquary_message}}</td>
                        <td>{{$consumer->ams_name}}</td>
                        <td>
                            @if(!empty($consumer->remark_id))
                        ...
                        @endif
                            ...
                            <button wire:confirm="Are you sure you want to delete this Consumer Enquiry ?" wire:click="delete({{ $consumer->genral_enquary_id }})" class="btn-danger"><i class="fa fa-trash fa-sm"></i></button>
                        </td>
                    </tr>
                    @php
                    $srno++
